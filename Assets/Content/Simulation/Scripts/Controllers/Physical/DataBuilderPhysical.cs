...
public class DataBuilderPhysical : MonoBehaviour
{

    public DataRiskPhysical dataRisk;
    private FlowControllerPhysical flowController;
    private int finishedAreas = 0;
    //public DataPlayer dataPlayer = new DataPlayer();

    void Start()
    {
        flowController = transform.GetComponent<FlowControllerPhysical>();
        //LECTURA DE ARCHIVOS DESDE LA BASE DE DATOS, ANTES DE INICIAR SIMULACION        
        if (GameObject.Find("SceneController") != null)
            dataRisk = new DataRiskPhysical(flowController.randomBusiness);
        //else
    ...
    public void finishedArea(){
        finishedAreas++;
    }
    public bool checkFinishedAreas(){
        Debug.Log(finishedAreas);   
        if(finishedAreas == 8){
            return true; 
        }
        else{
